...
export function AuthModal({ onClose, upsellMessage }: AuthModalProps) {
  const { login, signup, mockSocialLogin } = useAuth();
  ...
  const handleSocialClick = async (provider: "google" | "discord") => {
    setError(null);
    setLoading(true);
    try {
